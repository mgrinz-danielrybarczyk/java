...
public class Typing {
    ...
    public static void main(String[] args) {
        ...
        {
			playMethod(data, ';', 6_000);		
        }
    }
	
	public static void playMethod(String data, char separator, long limitTime) {
		long startTime = System.currentTimeMillis();
		
		double userPoints = 0;
		
		// *** MAXYMALNA MOŻLIWA LICZBA PUNKTÓW ***
		double maxPoints = maxPointsCounter(data, separator);
		System.out.println("Maksymalna liczba punktów do zdobycia: " + maxPoints);
		...
		private static double maxPointsCounter(String data, char separator) {
			String currentWord  = "";
			double maxPoints = 0;
			for (int i = 0; i < data.length(); i++) {
				char c = data.charAt(i);
		...
		private static int drawThePosition(String data, char separator) {
			int position;
			Random r = new Random();
			int lastIndexOfSeparator = data.lastIndexOf(separator);
			position = r.nextInt(lastIndexOfSeparator);
		...
		private static String displayTheWord(String data, char separator, int position) {
			String text;
			int indexOfSeparator = data.indexOf(separator, position);
			int indexOfNextSeparator = data.indexOf(separator, indexOfSeparator + 1);
			if (indexOfNextSeparator == -1) {
		...
		private static double pointsCounter(String text) {
			double points;
			Scanner scanner = new Scanner(System.in);
			System.out.println(text);
			String userText = scanner.nextLine();
